...
<body>
	...
	User user1 =null;
		...
		if(bulletinID == 0){
			...
			script.println("location.href = 'bulletinIntroduction.jsp'");
			script.println("</script>");
		}
		if(session.getAttribute("user") == null){
			PrintWriter script = response.getWriter();
		...
		<div class = "row">
				...
				<table class = "table table-bordered view">
					...
					<tbody>
						...
						<tr>
							<td  colspan='2'><img src="/upload/<%=bulletin.getFileRealName()%>" alt="이미지 없음" onerror="this.src='/css/images/logo.png'"class="bulletin-view-image"></td>
				...
				<form method = "post" action="mobile_bulletinIntroductionReplyAction.jsp?bulletinID=<%=bulletinID%>">
				...
				<table class = "table table-bordered replied-cover">
					...
					<tbody>
						<tr>
							<td class="replied-name"><%=userDAO.getUserName1(list1.get(i).getUserID())%></td>
							<td class="replied-date"><%=list1.get(i).getReplyDate()%>
						</tr>
				...
				<a href = "mobile_bulletinIntroduction.jsp" class = "btn btn-primary">목록으로</a>
